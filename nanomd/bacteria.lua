...
function Bacteria:__init(x, y)
	self.x = x
	self.y = y
	self.xspeed = math.random(-50, 50)
	self.yspeed = math.random(-50, 50)
	self.width = bacteriaGfx:getWidth()
	self.height = bacteriaGfx:getHeight()
	self.offsetX = self.width/2
	self.offsetY = self.height/2
    self.r = math.random(0.0, 6.28)
end
...
function Bacteria:draw()
	...
	scale = 1
	love.graphics.draw(bacteriaGfx, self.x, self.y, self.r)
end
